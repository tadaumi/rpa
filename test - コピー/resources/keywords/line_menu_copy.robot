...
Text Has Copy Like
    ...
    ${result}=    Evaluate    ${has_copy_1} or ${has_copy_2} or ${has_copy_3} or ${has_copy_4} or ${has_copy_5} or ${has_copy_6} or ${has_copy_7}
    Trace    [COPY-LIKE] norm=[${norm}] result=${result} h1=${has_copy_1} h2=${has_copy_2} h3=${has_copy_3} h4=${has_copy_4} h5=${has_copy_5} h6=${has_copy_6} h7=${has_copy_7}
    RETURN    ${result}
...
Resolve Bubble RightClickPoint
    ...
    ${raw_x}=    Evaluate    int(${l} + (${width} * 0.50))
    ${raw_y}=    Evaluate    int(${t} + (${height} * 0.50))
    ...
    ${x}=    Clamp To Range    ${raw_x}    ${min_x}    ${max_x}
    ${y}=    Clamp To Range    ${raw_y}    ${min_y}    ${max_y}
...
Copy One Message By BubbleRect
    ...
    IF    ${has_copy_like}
        ...
        Desk.Press Keys    esc
        RETURN    ${STATUS_CLIPBOARD_EMPTY}    ${text_candidate}    clipboard_empty    ${menu_text_trim}    ${menu_rect}    ${x}    ${y}
    END
    ...
    IF    ${has_delete}
        Trace    [COPY] move skipped: delete_present_but_no_copy_like
        Desk.Press Keys    esc
        RETURN    ${STATUS_MENU_NO_COPY}    ${text_candidate}    delete_present_but_no_copy_like    ${menu_text_trim}    ${menu_rect}    ${x}    ${y}
    END
